...
public class DesUtils {
    ...
    // paypwd密钥
    private final static String payKey = "N%n!27k@DTBoo4EC9d+1YrmwciaOn4rF!d9WxYLgVWVa$rGJ4gPYT&W011#aZ-19";

    public static String des3EncodePay(String plainText) {
        byte[] encryptData = new byte[0];
        try {
            Key deskey;
            DESedeKeySpec spec = new DESedeKeySpec(payKey.getBytes());
            SecretKeyFactory keyfactory = SecretKeyFactory.getInstance("desede");
            deskey = keyfactory.generateSecret(spec);
    ...
    public static String des3DecodePay(String encryptText) throws Exception {
        Key deskey;
        DESedeKeySpec spec = new DESedeKeySpec(payKey.getBytes());
        SecretKeyFactory keyfactory = SecretKeyFactory.getInstance("desede");
        deskey = keyfactory.generateSecret(spec);
    ...
    public static String base64Encode(byte[] data) {
        int start = 0;
        int len = data.length;
        StringBuilder stringBuilder = new StringBuilder(data.length * 3 / 2);
        ...
        while (i <= end) {
            ...
            stringBuilder.append(legalChars[(d >> 18) & 63]);
            stringBuilder.append(legalChars[(d >> 12) & 63]);
            stringBuilder.append(legalChars[(d >> 6) & 63]);
            stringBuilder.append(legalChars[d & 63]);
...
//        return new DesUtils().strEnc(data, "g", "z", "sq");
        return des3Encode(data);
    }
    ...
    public static String deString(String data) {
        try {
//            return new DesUtils().strDec(data, "g", "z", "sq");
            return des3Decode(data);
        } catch (Exception e) {
            LogUtils.e(e);
        }
        return "";
    ...
    private String strDec(String data, String firstKey, String secondKey,
                          String thirdKey) throws Exception {
        int leng = data.length();
        String decStr = "";
        List firstKeyBt = null, secondKeyBt = null, thirdKeyBt = null;
        int firstLength = 0, secondLength = 0, thirdLength = 0;
