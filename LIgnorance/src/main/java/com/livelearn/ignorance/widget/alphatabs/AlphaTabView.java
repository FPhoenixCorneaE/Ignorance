package com.livelearn.ignorance.widget.alphatabs;
...
public class AlphaTabView extends View {
    ...
    private float mBadgeNumberTextSize = 10;             //角标数大小
    ...
    private void drawBadge(Canvas canvas) {
        ...
        if (mBadgeNumber > 0) {
            ...
            float textSize = mBadgeNumberTextSize;
            int width;
            int hight = (int) dp2px(mContext, textSize);
            Bitmap bitmap;
            if (number.length() == 1) {
                width = (int) dp2px(mContext, textSize);
                bitmap = Bitmap.createBitmap(width, hight, Bitmap.Config.ARGB_8888);
            } else if (number.length() == 2) {
                width = (int) dp2px(mContext, textSize + 5);
                bitmap = Bitmap.createBitmap(width, hight, Bitmap.Config.ARGB_8888);
            } else {
                width = (int) dp2px(mContext, textSize + 10);
    ...
    public AlphaTabView setBadgeNumberTextSize(float mBadgeNumberTextSize) {
        this.mBadgeNumberTextSize = mBadgeNumberTextSize;
        return this;
    }
