...
class SmoothMarkDrawerSwitch extends SmoothMarkDrawer {
...
//	private static final int DEFAULT_COLOR_TRACK_ON = 0x60bdbdbd;
    ...
    SmoothMarkDrawerSwitch(Context context, int colorOn, int colorOff) {
...
//        canvas.drawRoundRect(mTrackRectF, mTrackRectF.height() / 2f, mTrackRectF.height() / 2f, mPaint);
        //轨道包裹拇指
        canvas.drawRoundRect(mBounds, mBounds.height() / 2f, mBounds.height() / 2f, mPaint);
