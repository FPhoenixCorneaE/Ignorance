package com.livelearn.ignorance.widget.tabstrip;
...
/**
 * ViewPager 小点,子项建议不超过5个
 *
 * @author Alex
 */
@ViewPager.DecorView
public class OvalTabStrip extends BaseTabStrip {
    ...
    public OvalTabStrip(Context context) {
        this(context, null);
    }

    public OvalTabStrip(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public OvalTabStrip(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        a.recycle();
        TypedArray custom = context.obtainStyledAttributes(attrs, R.styleable.OvalTabStrip);
        Drawable single = custom.getDrawable(R.styleable.OvalTabStrip_otsDrawable);
        padding = custom.getDimensionPixelSize(R.styleable.OvalTabStrip_otsDrawablePadding, padding);
        float scale = custom.getFloat(R.styleable.OvalTabStrip_otsScale, 1);
        custom.recycle();
        setGravity(gravity);
