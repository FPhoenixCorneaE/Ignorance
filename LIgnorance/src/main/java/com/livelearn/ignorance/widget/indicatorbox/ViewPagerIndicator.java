...
public class ViewPagerIndicator extends RippleCircle {
    ...
    private final ViewPager.OnPageChangeListener mInternalPageChangeListener = new ViewPager.OnPageChangeListener() {
        ...
        public void onPageSelected(int position) {
            if (mViewPager != null) {
                ((ViewPagerTextDrawer) patternDrawer).setmBigText(String.valueOf(position + 1));
            }
            startAnimationOnce();
        }
    ...
    private final DataSetObserver mInternalDataSetObserver = new DataSetObserver() {
        ...
        public void onChanged() {
            super.onChanged();
            if (mViewPager != null && mViewPager.getAdapter() != null) {
                ((ViewPagerTextDrawer) patternDrawer).setmSmallText(String.valueOf(mViewPager.getAdapter().getCount()));
                ((ViewPagerTextDrawer) patternDrawer).setmBigText(String.valueOf(mViewPager.getCurrentItem() + 1));
            }
            postInvalidate();
        }
    };
