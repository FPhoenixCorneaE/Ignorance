...
public class SmartTabLayout extends HorizontalScrollView {
  ...
  protected void onSizeChanged(int w, int h, int oldw, int oldh) {
    ...
    if (tabStrip.isIndicatorAlwaysInCenter() && tabStrip.getChildCount() > 0) {
      ...
      int start = (w - LayoutParamsUtils.getMeasuredWidth(firstTab)) / 2 - LayoutParamsUtils.getMarginStart(firstTab);
      int end = (w - LayoutParamsUtils.getMeasuredWidth(lastTab)) / 2 - LayoutParamsUtils.getMarginEnd(lastTab);
  ...
  private void scrollToTab(int tabIndex, float positionOffset) {
    ...
    final boolean isLayoutRtl = LayoutParamsUtils.isLayoutRtl(this);
    View selectedTab = tabStrip.getChildAt(tabIndex);
    int widthPlusMargin = LayoutParamsUtils.getWidth(selectedTab) + LayoutParamsUtils.getMarginHorizontally(selectedTab);
    ...
    if (tabStrip.isIndicatorAlwaysInCenter()) {
      ...
      if (0f < positionOffset && positionOffset < 1f) {
        ...
        int selectHalfWidth = LayoutParamsUtils.getWidth(selectedTab) / 2 + LayoutParamsUtils.getMarginEnd(selectedTab);
        int nextHalfWidth = LayoutParamsUtils.getWidth(nextTab) / 2 + LayoutParamsUtils.getMarginStart(nextTab);
      ...
      int x;
      if (isLayoutRtl) {
        int first = LayoutParamsUtils.getWidth(firstTab) + LayoutParamsUtils.getMarginEnd(firstTab);
        int selected = LayoutParamsUtils.getWidth(selectedTab) + LayoutParamsUtils.getMarginEnd(selectedTab);
        x = LayoutParamsUtils.getEnd(selectedTab) - LayoutParamsUtils.getMarginEnd(selectedTab) - extraOffset;
        x -= (first - selected) / 2;
      } else {
        int first = LayoutParamsUtils.getWidth(firstTab) + LayoutParamsUtils.getMarginStart(firstTab);
        int selected = LayoutParamsUtils.getWidth(selectedTab) + LayoutParamsUtils.getMarginStart(selectedTab);
        x = LayoutParamsUtils.getStart(selectedTab) - LayoutParamsUtils.getMarginStart(selectedTab) + extraOffset;
        x -= (first - selected) / 2;
      }
    ...
    if (titleOffset == TITLE_OFFSET_AUTO_CENTER) {
      ...
      if (isLayoutRtl) {
        x = -LayoutParamsUtils.getWidthWithMargin(selectedTab) / 2 + getWidth() / 2;
        x -= LayoutParamsUtils.getPaddingStart(this);
      } else {
        x = LayoutParamsUtils.getWidthWithMargin(selectedTab) / 2 - getWidth() / 2;
        x += LayoutParamsUtils.getPaddingStart(this);
      }
    ...
    int start = LayoutParamsUtils.getStart(selectedTab);
    int startMargin = LayoutParamsUtils.getMarginStart(selectedTab);
    if (isLayoutRtl) {
      x += start + startMargin - extraOffset - getWidth() + LayoutParamsUtils.getPaddingHorizontally(this);
    } else {
