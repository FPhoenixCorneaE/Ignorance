...
class SmartTabStrip extends LinearLayout {
  ...
  private void drawDecoration(Canvas canvas) {
    ...
    if (tabCount > 0) {
      ...
      int selectedStart = LayoutParamsUtils.getStart(selectedTab, indicatorWithoutPadding);
      int selectedEnd = LayoutParamsUtils.getEnd(selectedTab, indicatorWithoutPadding);
      int left;
      int right;
      if (isLayoutRtl) {
        left = selectedEnd;
      ...
      if (selectionOffset > 0f && selectedPosition < (getChildCount() - 1)) {
        ...
        int nextStart = LayoutParamsUtils.getStart(nextTab, indicatorWithoutPadding);
        int nextEnd = LayoutParamsUtils.getEnd(nextTab, indicatorWithoutPadding);
        if (isLayoutRtl) {
  ...
  private void drawSeparator(Canvas canvas, int height, int tabCount) {
    ...
    for (int i = 0; i < tabCount - 1; i++) {
      ...
      int end = LayoutParamsUtils.getEnd(child);
      int endMargin = LayoutParamsUtils.getMarginEnd(child);
