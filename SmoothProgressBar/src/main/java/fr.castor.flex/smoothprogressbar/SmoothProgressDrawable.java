package fr.castor.flex.smoothprogressbar;
...
/**
 * Created by fr.castor.flex on 11/10/13.
 */
public class SmoothProgressDrawable extends Drawable implements Animatable {
    ...
    private void checkColorIndex(int index) {
        if (index < 0 || index >= mColors.length) {
            throw new IllegalArgumentException(String.format(Locale.getDefault(),"Index %d not valid", index));
        }
    }
